...
#Rachit- introduced following variables for 2 types of validation
our @VAL;
my $f;
my $VALINDEX;
my $f2;
...
sub read_story()
{
	...
	# RACHIT-introduced ststements for SSF validation
	#system("dos2unix $_[0]");
	#print(STDERR "\nDo you want validated input(validated for SSF format)???\n");
	...
	{  
	   ...
	   if($v1=~/_/)
		{
			$VAL[1]=$`;  # $VAL[1] will contain the index of the feature structure the user claim to update
			$VAL[2]=$';  # $VAL[2] will contain the field which the user claim to update			 
	...
	{
	  $VAL[0]=$_[1];  # $VAL[0] will conatin the field number which the user claim to modify
	}  
	...
	for(my $i = 0; $i < scalar(@all_lines); $i++)
		...
		$count++;	
                ...
                {
			#Rachit-for SSF validation at doc level
				  if($inpvar==1)
					  {
						$PWD=`pwd`;
						chdir("$SSF_API/ssf_validator");
						system("perl lib/document_level.pl -i $storyname  -o Val_Error -m 0 -c config/log.conf");
						chdir($PWD);
						$fdoc=1;
						print(STDERR "\nJust ran document level validator....\n");
		...
		{	
			...
			$pnum = 0;
                ...
                {
				#Rachit-for SSF validation at paragraph level
					if($fdoc!=1 and $inpvar==1)
				
				{
				$PWD=`pwd`;
				chdir("$SSF_API/ssf_validator");
				system("perl lib/paragraph_level.pl -i $storyname -o Val_Error -m 0 -c config/log.conf");
				chdir($PWD);
				$fpara=1;
				print(STDERR "\nJust ran paragraph level validator....\n");
				}
...
#print STDERR "TB Number $pnum \n";
			...
			$count++;	
                ...
                {
				#Rachit-for SSF validation at sentence level
					if($fpara!=1 and $fdoc!=1 and $inpvar==1)
					{

					$PWD=`pwd`;
					chdir("$SSF_API/ssf_validator");
					system("perl lib/sentence_level.pl -i $storyname -o Val_Error -c config/log.conf");
					chdir($PWD);
					$fsent=1;
					print(STDERR "\nJust ran sentence level validator....\n");
					}
...
sub read()
{
	#Rachit-for SSF validation at ssf or word level level
	if($fsent!=1 and $fpara!=1 and $fdoc!=1 and $inpvar==1)
					{
						$PWD=`pwd`;
						chdir("$SSF_API/ssf_validator");
						system("perl lib/fs_level.pl -i $storyname -o Val_Error -m 0 -c config/log.conf");
						chdir($PWD);
						print(STDERR "\njust ran fs level validator ....\n");
					}
	
	if($inpvar==1)
		{
		  $PWD=`pwd`; # validation for ssf content 
		  chdir("$SSF_API/ssf_validator");
		  system("perl lib/content_fs.pl -i $storyname -o Val_Error -c config/log.conf");
		  chdir($PWD);
						
		  print(STDERR "\njust ran fs content level validator ....\n\n");
	...
	foreach (@$sent_ref)
	{
		...
		if($_TREE_[$nElements][4] eq "" and $_TREE_[$nElements][2] ne "))")
		{
			$_TREE_[$nElements][4]="<fs >";
...
sub assign_readable_numbers()
{
	...
	{	
		#Rachit-$f is set to 1 in order to distinguish "modify function" call by user and other programs of this file
		$f=1;
...
sub get_field()
{
	...
	if($$TreeRef[$node][$index]==0 and $index==4)
		{
			$fadd_attr_val=1;
			#$$TreeRef[$node][$index]="<fs >"	
		}
	#print("\n$$TreeRef[$node][$index]\n");
...
sub modify_field()
{
	...
	# Rachit-Validation checks are made here.If the field value as supplied by the user with the read function does not #matches with the 2nd argument of this function(i.e the filed value supplied with the modify field function) then it #intimates the user and proceeds as per users response..

	if($f==1 or $index==4 )
	{
		$$TreeRef[$node][$index] = $value; #when this function is called by some other functions of this file
		return;
	}
	else
	{
		...
		if($VAL[0] eq "")
			{
			    	...
			    	print(STDERR "\nYou did not provide the 2nd argument with the read function..!!\n");
				goto rac;
			
			}
		if($index==$VAL[0] && $VAL[0]!~/fs/)
			{
				$$TreeRef[$node][$index] = $value;
			}
		else
			{	
				print(STDERR "\nU are trying to change a different field than what is specified with the read function....!!");
			    rac:print(STDERR "\nwhat would you like to do...!!\n");
				print(STDERR "\nenter 1 to continue without modifying...!!\n");
				print(STDERR "\nenter 2 to forcibly modify the value...!!\n");
