...
public class SVMToy extends Applet {
	...
	static final String DEFAULT_PARAM=GlobalProperties.getIntlString("-t_2_-c_100");
	int XLEN;
	int YLEN;
	...
	public void init()
	{
		...
		final Button button_change = new Button(GlobalProperties.getIntlString("Change"));
		Button button_run = new Button(GlobalProperties.getIntlString("Run"));
		Button button_clear = new Button(GlobalProperties.getIntlString("Clear"));
		Button button_save = new Button(GlobalProperties.getIntlString("Save"));
		Button button_load = new Button(GlobalProperties.getIntlString("Load"));
	...
	void button_run_clicked(String args)
	{
		...
		for(int i=0;i<argv.length;i++)
		{
			...
			if(++i>=argv.length)
			{
				System.err.print(GlobalProperties.getIntlString("unknown_option\n"));
				break;
			}
	...
	{
		FileDialog dialog = new FileDialog(new Frame(),GlobalProperties.getIntlString("Save"),FileDialog.SAVE);
		...
		try {
			...
			{
				point p = point_list.elementAt(i);
				fp.writeBytes(p.value+GlobalProperties.getIntlString("_1:")+p.x+GlobalProperties.getIntlString("_2:")+p.y+GlobalProperties.getIntlString("\n"));
			}
			fp.close();
	...
	{
		FileDialog dialog = new FileDialog(new Frame(),GlobalProperties.getIntlString("Load"),FileDialog.LOAD);
		dialog.setVisible(true);
		String filename = dialog.getFile();
		if (filename == null) return;
		clear_all();
	...
	{
		new AppletFrame(GlobalProperties.getIntlString("SVMToy"),new SVMToy(),500,500+50);
	}
}
