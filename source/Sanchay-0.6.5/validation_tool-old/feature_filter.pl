...
sub get_values_2()  # modified by Rachit-
{
	...
	else
	{
		...
		for(my $i=0;$i<@$arrayRef;$i++)
		{
			...
			else
			{
				...
				push(@RetArray,$$arrayRef[$i]);
				
				
				push(@RetArray,&get_values_2($nextPath,$hashRef));
...
sub get_attributes()
{
	...
	return @attributes;
#	print "\n\n\nINSIDE APIS......@attributes.....";
#	print "**************",$$hashRef{"drel"},"***********";
#	print $$hashRef{$attributes[0]},"++++++";
...
#		if(scalar(&get_values($attributes[$i],$hashRef)) > 0)
#		{
			#print "*",&get_values($attributes[$i],$hashRef),"*";
#			$differentArray[$j]=$attributes[$i];
#			print "INSIDE APIS---------......@attributes[$i].....\n\n\n";
#			$j++;
#		}
		#print "*",$differentArray,"*";
#	}
...
{
	my $featurePath=$_[0];
	my $val=$_[1]; # Rachit-						# This value has to be a reference to an array.
...
# Rachit-
  ...
  if($f2!=1)
   ...
   if($f2!=1)
   { 
    if(($VAL[2] ne $_[0]) or ($VAL[1]!=($VALINDEX+1) ) or $kfs ne "fs")
	 {
	...
	if($nextPath eq "")
	{
		if(defined($$hashRef{$presAttr}))		# Update an existing value.
		{
			...
			{	
				...
				{	
					push(@arrayAdd1,$$val[$i]);
					
					
				}
			}
			
			$$hashRef{$presAttr}=\@arrayAdd1;
	...
	else
	{
		...
		for(my $i=0;$i<@$arrayRef;$i++)
		{
			...
			else
			{
				...
				$$hashRef{$presAttr}=\@RetArray;
				
				shift(@$val);
				
				$f2=1;
				&update_attr_val_2($nextPath,\@$val,$hashRef);
			}
